...
def computeODF(inputFile, window, M, N, H):
    ...
    # Step 2: Perform STFT analysis
    w = get_window(window, M)
    mX, pX = stft.stftAnal(x, w, N, H)  # Magnitude and phase spectrum

    # Step 3: Determine frequency bins for low and high frequency bands
    freq_bin = np.arange(N//2 + 1) * fs / N
    low_band_idx = np.where((freq_bin > 0) & (freq_bin < 3000))[0]
    high_band_idx = np.where((freq_bin > 3000) & (freq_bin < 10000))[0]

    # Step 4: Calculate the energy for each band
    low_band_energy = np.sum(mX[:, low_band_idx] ** 2, axis=1)
    high_band_energy = np.sum(mX[:, high_band_idx] ** 2, axis=1)

    # Step 5: Calculate the Onset Detection Function (ODF)
    # Initial condition ODF(0) = 0
    low_band_odf = np.diff(np.insert(low_band_energy, 0, 0))
    high_band_odf = np.diff(np.insert(high_band_energy, 0, 0))

    # Step 6: Apply half-wave rectification
    low_band_odf[low_band_odf < 0] = 0
    high_band_odf[high_band_odf < 0] = 0

    # Step 7: Return the ODF as a numpy array
