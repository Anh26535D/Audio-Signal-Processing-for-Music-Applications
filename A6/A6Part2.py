...
def segmentStableNotesRegions(inputFile = '../../sounds/sax-phrase-short.wav', stdThsld=10, minNoteDur=0.1, 
    ...
    for i in range(f0std.size):
        f0std[i] = np.std(f0cents[i:i+winStable])
    ...
    for i in range(f0std.size-1):
        if f0std[i] == -9999999:
            if not increase:
                start = i
                increase = True
            if f0std[i+1] != -9999999:
                end = i
                increase = False
                segLen = (end-start+1)*x.size/f0.size
                if (segLen >= (minNoteDur*44100)):
                    # since the index of should be forwarded by winStable frame
                    # the index is added by the winStable-1
                    segg.append([start+winStable-1, end+winStable-1])

    segments = np.array(segg)
