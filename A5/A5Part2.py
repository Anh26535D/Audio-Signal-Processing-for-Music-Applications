...
def chirpTracker(inputFile='../../sounds/chirp-150-190-linear.wav'):
    ...
    # FFT Size, power of 2 larger than M
    N = int(pow(2, np.ceil(np.log2(M))))
    t = -80.0                                   # threshold
    window = 'blackman'                         # Window type
    # Maximum number of sinusoids at any time frame
    maxnSines = 2
    # minimum duration set to zero to not do tracking
    minSineDur = 0.0
    ...
    # Generate the tStamps to return
    tStamps = genTimeStamps(x.size, M, fs, H)
    # analyze the sound with the sinusoidal model
    fTrackEst, mTrackEst, pTrackEst = SM.sineModelAnal(
        x, fs, w, N, H, t, maxnSines, minSineDur, freqDevOffset, freqDevSlope)
    # Generate the true frequency tracks
    fTrackTrue = genTrueFreqTracks(tStamps)
    tailF = 20
    # Compute mean estimation error. 20 frames at the beginning and end not used to compute error
    meanErr = np.mean(
        np.abs(fTrackTrue[tailF:-tailF, :] - fTrackEst[tailF:-tailF, :]), axis=0)
    print("Mean estimation error = " + str(meanErr) +
          ' Hz')      # Print the error to terminal
    # Plot the estimated and true frequency tracks
    mX, pX = stft.stftAnal(x, w, N, H)
    maxplotfreq = 1500.0
    binFreq = fs*np.arange(N*maxplotfreq/fs)/N
    plt.pcolormesh(tStamps, binFreq, np.transpose(
        mX[:, :int(N*maxplotfreq/fs+1)]), cmap='hot_r')
