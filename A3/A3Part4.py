...
def suppressFreqDFTmodel(x, fs, N):
    ...
    ## Your code here
    # Analyzing the input signal using dftAnal
    mX, pX = dftAnal(x, w, N)
    
    # Finding the bin corresponding to 70 Hz
    cutoff_bin = np.ceil(70 / (fs/N)).astype(int)
    
    # Create a copy of the magnitude spectrum for filtering
    mX_filt = mX.copy()
    
    # Suppress the components <= 70 Hz by setting their magnitude to -120 dB
    mX_filt[:cutoff_bin + 1] = -120
    
    # Synthesize the output signals
    y = dftSynth(mX, pX, N) * outputScaleFactor
    yfilt = dftSynth(mX_filt, pX, N) * outputScaleFactor
